...
namespace DMValidator
{
    ...
    class IoTHubManager
    {
        ...
        public const int DirectMethodFailureCode = 400;

        static private string DeviceTwinDesiredObject = "{{ \"properties\": {{ \"desired\": {0} }} }}";
        ...
        public async Task UpdateDesiredProperty(string deviceId, string name, object value)
        {
            string propertiesString = "{ \"" + name + "\": " + ((value != null) ? value.ToString() : "null") + "}";
            await UpdateDesiredObject(deviceId, propertiesString);
        }

        public async Task UpdateDesiredObject(string deviceId, object value)
        {
            string updateJson = String.Format(DeviceTwinDesiredObject, value.ToString());

            Debug.WriteLine(updateJson);
            ...
            if (registryManager != null)
            {
                try
                {
                    ...
                    {
                        Console.WriteLine(messageDeviceTwinFunctionalityNotFound, "Device Twin Properties Update");
                    }
                }
                catch (Exception ex)
                {
                    string errMess = "Update Twin failed. Exception: " + ex.ToString();
                    Console.WriteLine(errMess, "Device Twin Desired Properties Update");
                }
            }
            else
            {
                Console.WriteLine("Registry Manager is no initialized!", "Device Twin Desired Properties Update");
            }
        }
        ...
        public async Task<DeviceData> GetDeviceData(string deviceId)
        {
            ...
            {
                Console.WriteLine(ex.Message + Environment.NewLine + "Make sure you are using the latest Microsoft.Azure.Devices package.", "Device Twin Properties");
            }
            return result;
        }
