...
namespace Microsoft { namespace Azure { namespace DeviceManagement { namespace WindowsTelemetryPlugin {
    ...
    WindowsTelemetryStateHandler::WindowsTelemetryStateHandler() :
        BaseHandler(WindowsTelemetryStateHandlerId, ReportedSchema(JsonDeviceSchemasTypeRaw, JsonDeviceSchemasTagDM, 1, 1))
    {
    }
        ...
        default:
            {
                ...
                throw DMException(ERROR_DM_WINDOWS_TELEMETRY_INVALID_LEVEL, msg.str().c_str());
            }
        }
    ...
    {
        RunOperation(JsonWindowsTelemetryLevel, errorList,
            [&]()
        {
            unsigned long level;
            if (ERROR_SUCCESS != Registry::TryReadRegistryValue(RegWindowsTelemetrySubKey, RegAllowTelemetry, level))
            {
                throw DMException(ERROR_DM_WINDOWS_TELEMETRY_MISSING_LEVEL, "Error: User setting for Windows telemetry level is not defined.");
            }
        ...
        std::shared_ptr<DMCommon::ReportedErrorList> errorList)
    {
        RunOperation(JsonWindowsTelemetryLevel, errorList,
            [&]()
        {
            string levelString = GetSinglePropertyOpStringParameter(groupDesiredConfigJson, JsonWindowsTelemetryLevel);
