...
namespace Microsoft { namespace Azure { namespace DeviceManagement { namespace Plugin { namespace Common {
    ...
    void PluginStub::Initialize()
    {
        ...
        _rawHandlerHostProxy = make_shared<RawHandlerHostProxy>(&_agentBinaryProxy);

        _rawHandlerStub.SetAgentBinaryProxy(&_agentBinaryProxy);
        _rawHandlerStub.SetMdmServerProxy(_mdmServerProxy);
        ...
        const Json::Value& targetParameters)
    {
        ...
        if (targetType == JsonTargetTypeHost)
        {
            ...
            {
                throw DMException(PLUGIN_ERR_INVALID_PLUGIN_HOST_OPERATION, "Unknown host operation.");
            }
        ...
        throw DMException(PLUGIN_ERR_INVALID_PLUGIN_OPERATION, "Unknown host operation.");
    }
