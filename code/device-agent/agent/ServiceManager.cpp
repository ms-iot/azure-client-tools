...
DWORD ServiceManager::GetStatus(const std::wstring& serviceName)
{
    ...
    {
        throw DMException(DMSubsystem::Windows, GetLastError(), "OpenSCManager() failed.");
    }
    ...
    {
        throw DMException(DMSubsystem::Windows, GetLastError(), "OpenService() failed.");
    }

    SERVICE_STATUS serviceStatus;
    if (!QueryServiceStatus(serviceHandle.get() /*service handle*/, &serviceStatus))
    {
        throw DMException(DMSubsystem::Windows, GetLastError(), "QueryServiceStatus() failed.");
    }
...
DWORD ServiceManager::GetStartType(const std::wstring& serviceName)
{
    ...
    {
        throw DMException(DMSubsystem::Windows, GetLastError(), "QueryServiceConfig() failed.");
    }
...
void ServiceManager::StartStop(const std::wstring& serviceName, bool start)
{
    ...
    if (start)
    {
        ...
        {
            throw DMException(DMSubsystem::Windows, GetLastError(), "StartService() failed.");
        }
    ...
    else
    {
        ...
        {
            throw DMException(DMSubsystem::Windows, GetLastError(), "ControlService() failed.");
        }
...
void ServiceManager::SetStartType(const std::wstring& serviceName, DWORD startType)
{
    ...
    {
        throw DMException(DMSubsystem::Windows, GetLastError(), "ChangeServiceConfig() to change service to auto start.");
    }
}
