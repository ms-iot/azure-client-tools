...
namespace Microsoft { namespace Azure { namespace DeviceManagement { namespace Client {
        ...
        const Json::Value& targetParameters)
    {
        if (NULL == PluginInvoke)
        {
            throw DMException(DMSubsystem::DeviceAgent, DM_PLUGIN_ERROR_MISSING_ENTRY_POINT, "Failed to find entry point PluginInvoke.");
        }
        ...
        {
            throw DMException(DMSubsystem::DeviceAgent, retCode, "PluginInvoke returned error.");
        }
