...
namespace Microsoft { namespace Azure { namespace DeviceManagement { namespace Common {
    ...
    Json::Value CrossBinaryResponse::GetPayloadFromJsonString(const std::string& responseJsonString)
    {
        ...
        {
            throw DMException(DMSubsystem::DeviceAgent, DM_ERROR_INVALID_RESPONSE_JSON, "Failed to parse json to construct CrossBinaryResponse.");
        }

        if (jsonObject.isNull() || !jsonObject.isObject())
        {
            throw DMException(DMSubsystem::DeviceAgent, DM_ERROR_INVALID_RESPONSE_JSON_SCHEMA, "Failed to parse plug-in response: invalid type.");
        }

        Json::Value successValue = jsonObject[JsonCrossBinaryResponseSuccess];
        if (successValue.isNull() || !successValue.isBool())
        {
            throw DMException(DMSubsystem::DeviceAgent, DM_ERROR_INVALID_RESPONSE_JSON_SCHEMA, "Failed to parse plug-in response: invalid type or missing 'success'.");
        }
