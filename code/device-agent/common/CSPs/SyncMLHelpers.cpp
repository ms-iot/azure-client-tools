...
namespace Microsoft { namespace Azure { namespace DeviceManagement { namespace Common {
    ...
    void ReadXmlStructData(IStream* resultSyncML, ELEMENT_HANDLER handler)
    {
        ...
        if (FAILED(hr))
        {
            throw DMException(DMSubsystem::XmlLite, hr, "Error: Failed to create xml reader.");
        }

        hr = xmlReader->SetProperty(XmlReaderProperty_DtdProcessing, DtdProcessing_Prohibit);
        if (FAILED(hr))
        {
            throw DMException(DMSubsystem::XmlLite, hr, "Error: XmlReaderProperty_DtdProcessing() failed.");
        }

        hr = xmlReader->SetInput(resultSyncML);
        if (FAILED(hr))
        {
            throw DMException(DMSubsystem::XmlLite, hr, "Error: SetInput() failed.");
        }
        ...
        while (S_OK == (hr = xmlReader->Read(&nodeType)))
        {
            ...
            case XmlNodeType_Element:
            {
                ...
                if (FAILED(hr))
                {
                    throw DMException(DMSubsystem::XmlLite, hr, "Error: GetPrefix() failed.");
                }
                ...
                if (FAILED(hr))
                {
                    throw DMException(DMSubsystem::XmlLite, hr, "Error: GetLocalName() failed.");
                }
            ...
            case XmlNodeType_Whitespace:
            {
                ...
                if (FAILED(hr))
                {
                    throw DMException(DMSubsystem::XmlLite, hr, "Error: GetValue() failed.");
                }
    ...
    void ReadXmlValue(IStream* resultSyncML, const wstring& targetXmlPath, wstring& value)
    {
        ...
        if (!pathFound)
        {
            ...
            throw DMException(DMSubsystem::DeviceAgent, DM_ERROR_MISSING_XML_PATH, WideToMultibyte(message.c_str()));
        }
    }

    void ReadXmlStatus(IStream* resultSyncML, const wstring& targetXmlPath, vector<unsigned int>& returnCodes)
    {
        wstring resultPath = L"SyncML\\SyncBody\\Results\\";
        ComPtr<IXmlReader> xmlReader;
        ...
        bool pathFound = false;
        XmlNodeType nodeType;
        while (S_OK == (hr = xmlReader->Read(&nodeType)) && !(currentPath == resultPath))
        {
            switch (nodeType)
            {
            case XmlNodeType_Element:
            ...
            case XmlNodeType_Whitespace:
            {
                ...
                if (targetXmlPath == currentPath)
                {
                    returnCodes.push_back(stoi(valueText));
                }
            }
            break;
            }
    ...
    void ReadXmlStructData(const wstring& resultSyncML, ELEMENT_HANDLER handler)
    {
        ...
        {
            GlobalFree(buffer);
            throw DMException(DMSubsystem::Windows, hr, "CreateStreamOnHGlobal call failed.");
        }
    ...
    void ReadXmlValue(const wstring& resultSyncML, const wstring& targetXmlPath, wstring& value)
    {
        ...
        {
            GlobalFree(buffer);
            throw DMException(DMSubsystem::Windows, hr, "Failed to create stream object" );
        }
    ...
    void ReadXmlStatus(const wstring& resultSyncML, const wstring& targetXmlPath, vector<unsigned int>& returnCodes)
    {
        DWORD bufferSize = static_cast<DWORD>(resultSyncML.size() * sizeof(resultSyncML[0]));
        char* buffer = (char*)GlobalAlloc(GMEM_FIXED, bufferSize);
        memcpy(buffer, resultSyncML.c_str(), bufferSize);
        ...
        ReadXmlStatus(dataStream.Get(), targetXmlPath, returnCodes);
