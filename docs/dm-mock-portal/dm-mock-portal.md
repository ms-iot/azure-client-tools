...
For example, let's say we want to set the telemetry level to `basic` for arm devices, while we want to set it to `security` for x86 device. To do that we will:
...
For example, we have [adm.windows_telemetry_x86_v1.json](adm.windows_telemetry_x86_v1.json) and [adm.windows_telemetry_arm_v1.json](adm.windows_telemetry_arm_v1.json) for your convenience.
...
- To create a deployment, click the `+`.
    ...
    - You can import a pre-existing deployment profile by clicking `Import` then `Save`.
        - The repo comes with a set of pre-defined deployment profiles:
            ...
            - [adm.windows_telemetry_x86_v1.json](adm.windows_telemetry_x86_v1.json)
